...
import lyzh.syntax.{Parser, Prog}
import lyzh.Lyzh
...
object SubstMltt {
  ...
  def loadProg(path: String) = parse(NodeFs.readFileSync(path, "utf-8"))
...
class LyzhMlttTests extends munit.FunSuite {
  ...
  import syntax._
  import lyzh.Utils.{debugLyzh as debugln}
  ...
  test("readme") { testit("samples/readme.lyzh") }
  // test("predef") { testit("samples/predef.lyzh") }
  // test("exts") { testit("samples/exts.mltt") }
}
